...
class PaypalForm extends ContentEntityForm {
   ...
   */
  private $paypal_payments_entity_save; #TODO:: use this service to do some more stuff

  public function __construct(\Drupal\Core\Entity\EntityRepositoryInterface $entity_repository, paypalSettings $paypal_settings, \Drupal\Core\Entity\EntityTypeBundleInfoInterface $entity_type_bundle_info = NULL, \Drupal\Component\Datetime\TimeInterface $time = NULL) {
    ...
    $field_definitions = \Drupal::service('entity_field.manager')->getFieldDefinitions('node', $bundle);
     ...
     foreach ($field_definitions as $key => $field) {
       if ($field->getType() == 'field_paypal'){
         ...
         $price = $this->getPrice($paypalPrice);
   ...
   *
   */
  protected function getPrice($paypalPrice){
   ...
   * @param \Drupal\node\Entity\Node $node
   */
  protected function generateSkuFromNodeTitle(Node $node){
    $title = $node->getTitle();
